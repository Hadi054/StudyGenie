...
function prevp(){
  if (pageno.innerText.toString() > "1") {
    ...
    window.location.href = `/notes?page=${page}&class=${selectedclass}&sub=${selectedsub}&search=${encodeURIComponent(searchQuery)}`;
  }
};
...
function readnow(Bookid){
  console.log("Hi")
window.open(`/getNote?id=${Bookid}`);

}
function download(Bookid){
  console.log("Hi")
window.open(`/downloadnote?id=${Bookid}`);
