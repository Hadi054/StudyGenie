...
const Blog = require("../models/people.js").v5;
...
console.log(filter)
    ...
    Blog.find(filter)
      ...
      .then((blog) => {
        ...
        for (var i = 0; i < blog.length; i++) {
          // blog[i].img.data=Buffer.from(blog[i].img.data).toString('base64')
          blog[i].shorttitle=getFirstWords(blog[i].name, 5);
        ...
        if(req.payload==0)
        {
            res.render("newsletter",{popularblog,blog,currentPage,maxPage,filterset,category,
                isloggedin:false}); 
        }
        else
        { if(extra.img.data)
          extra.img.data=Buffer.from(extra.img.data).toString('base64')
            res.render("newsletter",{popularblog,userinfo,extra,blog,currentPage,maxPage,filterset,category,
            isloggedin:true
        });
    }
         });
