...
const Book = require("../models/people.js").v6;
...
router.get("/",[verifyjwt], async (req,res)=>{
    ...
    Book.find(filter)
      ...
      .then((book) => {
        ...
        if(req.payload==0)
        {
            res.render("notes",{book,currentPage,subFilter,classFilter,maxPage,popularbook,
                isloggedin:false}); 
        }
        else
        { if(extra.img.data)
          extra.img.data=Buffer.from(extra.img.data).toString('base64')
            res.render("notes",{userinfo,extra,book,currentPage,subFilter,classFilter,maxPage,popularbook,
            isloggedin:true
        });
    }
         });
